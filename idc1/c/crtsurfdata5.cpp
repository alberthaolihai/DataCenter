...
int main(int argc,char *argv[])
{
	...
	logfile.Write("程序crtsuffdata5开始运行 \n");
	// 加载站点文件入容器;
...
bool CrtSurfFile(const char *outpath,const char *datafmt)
{
	...
	if(strcmp(datafmt,"xml")==0) {File.Fprintf("<date>\n");}
	
	if(strcmp(datafmt,"json")==0) {File.Fprintf("\"date\":[\n");}
	// 遍历存放观测数据的容器
	...
	if(strcmp(datafmt,"xml")==0) {File.Fprintf("</date>\n");}
	if(strcmp(datafmt,"xml")==0) {File.Fprintf("]}\n");}
	File.CloseAndRename();
